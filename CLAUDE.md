...
- **Hono** HTTP server on port 3333 with **tRPC** mounted at `/trpc/*`
- **Awilix** dependency injection: stores are registered in `core/container.ts` and accessed via `ctx.cradle` in tRPC procedures
...
- **tRPC routers** live in `app/router/`, composed into `appRouter` in `app/router/index.ts`
- **Schemas** (Zod) go in `app/schemas/`
- Path aliases: `#app/*` → `./app/*`, `#core/*` → `./core/*`
- The `AppRouter` type is exported from `@permitflow/backend` for frontend consumption

### Frontend (`frontend/`)

- **React 19** + **Vite** on port 6173 + **React Router 7** + **Tailwind CSS 4**
- **tRPC client** configured in `src/lib/trpc.ts` using `@trpc/tanstack-react-query` — use `trpc` proxy for type-safe queries/mutations with TanStack React Query
...
Backend tRPC router type (`AppRouter`) is imported by the frontend via the `@permitflow/backend` package. Changes to backend procedures automatically flow to frontend type checking.
