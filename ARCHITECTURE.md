...
- **Real database** -- Replace JSON file stores with Postgres (already in the devcontainer stack) for concurrent access and durability
- **Optimistic UI on draft save** -- Show a subtle "Saving..." / "Saved" indicator so users have confidence their work is persisted
- **Versioned question definitions** -- Store the question schema version with each submission so permit logic can evolve without breaking historical records
